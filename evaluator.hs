...
staticeval (State{board=b})  = sum $ map (\x -> if isNothing (fst x) then 0 else (value (fst x)) + (weightedposition (fst x) (snd x))) (Prelude.zip (toList b) [0..63])
...
value (Just (Piece King True))       = 9999
...
value (Just (Piece King False))      = -9999
